...
app.post('/ping', (req, res) => {
    ...
    log(`got pong from server two with status ${res.statusCode}`)
...
const sendNextPing = async (retries = 2) => {
    for (let i = 0; i < retries; i++) {
        try {
            await axios.post('http://localhost:3001/ping', { 
                message: 'ping',
                attempt: i + 1
            }, {
                timeout: gameState.delayMs
            })
            log(`${SERVER_NAME} pinged server two`)
            return true
        } catch (error) {
            ...
            if (error.code === 'ECONNREFUSED') {
                ...
                log(`${attemptMsg}: server_two timed out`, 'error')
            }
            else {
